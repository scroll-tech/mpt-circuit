...
impl WordColumns {
    ...
    pub fn current<F: PrimeField>(&self) -> [Query<F>; 2] {
        [self.hi.current(), self.lo.current()]
    }

    pub fn previous<F: PrimeField>(&self) -> [Query<F>; 2] {
        [self.hi.previous(), self.lo.previous()]
    }

    pub fn assign<F: PrimeField>(&self, region: &mut Region<'_, F>, offset: usize, value: U256) {
